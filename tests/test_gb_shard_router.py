"""Structural tests for the GB multi-shard engine router.
...
device-context entry, output scatter order, ``min_freq_inds`` pointer identity
across in-place parent updates, per-slot kwarg slicing, the per-device comp /
engine replicas (including the sig-het in-model reference isolation), and the
single-shard passthrough.

Ported from ``dev``'s ``tests/test_gb_shard_router.py`` (commits ``05645d2``,
``3f60ac4``, ``bd502af``, ``ed17e8b``). Differences on this branch:

* **No F-stat tests.** ``route_fstat_ll`` needs the B1 F-stat stack, deferred.
  ``route_information_matrix`` is the raw-comp entry point we do have, and it
  covers the same routing/replica machinery.
* **STFT coverage added.** This branch's ``STFTBandLikelihoodEngine`` shards
  internally, so ``make_routed_band_engine`` must leave it UNWRAPPED, and
  ``_ShardHolderView`` must override ``cpp_splits`` -- neither has an analogue
  on ``dev``.
* **The single-shard contract guard** on ``_as_wdm_holder`` is covered here
  too: it is what turns a multi-shard WDM holder from a silently wrong answer
  into a raise.
"""
...
class _StubEngine:
    ...
    Outputs encode (shard device, intra row) so the router's scatter order is
    verifiable: ``ll = 1000 * device + intra``.
    """
...
class _StubComp:
    ...
    entry point that bypasses the wrapped engine). Encodes (device, intra noise
    row) into the Fisher stack so routing + reassembly is verifiable.
    """
...
class _StubSigHetEngine:
    ...
    Mirrors ``GBSignalHetComputations.setup_in_model``: a flat ``_slot_to_ref``
    map over INTRA-shard slot ids, a coefficient stash keyed by reference row,
    and a single ``_in_model`` flag whose presence turns the next call from a
    fresh build into a mid-block PATCH of the existing references. That flat
    state is exactly what two shards collide over when they share one comp --
    their intra-shard slot ids both start at zero -- so reproducing it here is
    what makes the cross-shard corruption regression testable on CPU.
    """
...
class ShardRouterTest(unittest.TestCase):
    ...
    def test_shard_view_protocol(self):
        ...
        # intra-shard rank is the identity on a single-shard view
        np.testing.assert_array_equal(
            view.ac_to_intra, np.arange(len(self.holder.gpu_splits[1])))
    ...
    def test_cross_shard_noise_index_refused(self):
        """A row whose data band and noise band live on different shards has
        no single owning device -- refuse rather than pick one."""
        d_idx = np.array([0, 1])           # shards 0, 1 (striped)
        n_idx = np.array([0, 2])           # shards 0, 2 -> row 1 mismatches
        with self.assertRaises(ValueError):
            self.router.get_ll(
                self.holder, self.params[:2],
                data_index=d_idx, noise_index=n_idx,
                N_vals=None, waveform_kwargs={},
            )

    # ---------------- sig-het in-model routing ----------------
    ...
    def test_shard_view_slices_slab_min_f(self):
        """``slab_min_f`` is per BUFFER SLOT, so the view must hand each shard
        its OWN rows -- ``__getattr__`` delegation would return the parent's
        global-slot array against intra-shard indices."""
    ...
    # ---------------- STFT arm (branch-specific) ----------------

    def test_shard_view_cpp_splits_is_single(self):
        """``cpp_splits`` must be the shard's OWN one-element list.

        ``STFTGBComputations._resolve_info_group`` reads ``len(cpp_splits)``
        and raises ``NotImplementedError`` on anything but 1, so a delegated
        (parent, full-length) list would make ``route_information_matrix``
        fail on exactly the multi-shard holders it exists to serve.
        """
        self.holder.cpp_splits = [f"group{s}" for s in range(self.NUM_SHARDS)]
        try:
            for s in range(self.NUM_SHARDS):
                view = self.ShardView(self.holder, s)
                self.assertEqual(view.cpp_splits, [f"group{s}"])
                self.assertEqual(len(view.cpp_splits), 1)
        finally:
            del self.holder.cpp_splits

    def test_shard_view_cpp_splits_none_without_parent_groups(self):
        """FD / WDM holders carry no computation groups -> None, matching the
        ``getattr(holder, "cpp_splits", None)`` call sites."""
        self.assertIsNone(self.ShardView(self.holder, 0).cpp_splits)

    def test_stft_engine_is_not_wrapped(self):
        """``STFTBandLikelihoodEngine`` shards internally (``_split_plan``) and
        needs the REAL multi-shard ACA, so the factory must return it bare.

        Wrapping it would feed it a ``_ShardHolderView`` whose
        ``linear_data_arr`` is 1-element while its own per-split loop still
        ranges over ``cpp_splits`` -- double routing, and an index past the
        end.
        """
        try:
            from gbgpu.gb_likelihood import STFTBandLikelihoodEngine
        except (ImportError, ModuleNotFoundError) as exc:
            self.skipTest(f"gbgpu not available: {exc}")
        from lisatools.globalfit.moves.gbbands import (
            _RoutedBandEngine, make_routed_band_engine)

        # The STFT engine dispatches per split itself; confirm the contract we
        # rely on is actually present on the installed gbgpu.
        self.assertTrue(hasattr(STFTBandLikelihoodEngine, "_split_plan"))
        self.assertTrue(hasattr(STFTBandLikelihoodEngine, "_comp_for_split"))

        from lisatools.domains import STFTSettings

        class _FakeSTFTEngine:
            pass

        import gbgpu.gb_likelihood as gl

        real = gl.make_band_likelihood_engine
        settings = STFTSettings.__new__(STFTSettings)
        gl.make_band_likelihood_engine = (
            lambda *a, **k: _FakeSTFTEngine())
        try:
            out = make_routed_band_engine(settings, xp=np, gb_stft_comp=object())
        finally:
            gl.make_band_likelihood_engine = real
        self.assertIsInstance(out, _FakeSTFTEngine)
        self.assertNotIsInstance(out, _RoutedBandEngine)
    ...
    def test_information_matrix_dispatches_to_device_local_comp(self):
        """Each shard's call must reach the comp replica whose OWN build
        device matches the shard's (the comp stamps its build device into the
        Fisher diagonal, so a foreign-device comp is visible in the result)."""
        ...
        # NOTE: a non-primary shard runs on a REPLICA -- a separate object with
        # its own call log -- so ``comp.calls`` holds only the primary shard's
        # call. Walk the replica cache for the rest.
        from lisatools.utils.devicereplicas import _DEVICE_GB_COMP_REPLICAS
        ...
        for cdev, c in by_device.items():
            for call in c.calls:
                ...
                ran.add(int(cdev))
        self.assertEqual(ran, {int(self.holder.gpu_map[g]) for g in idx})
    ...
    def test_comp_replica_reused_across_calls(self):
        ...
        self.assertEqual(
            ...
            sorted({int(self.holder.gpu_map[g]) for g in idx} - {0}),
        )
    ...
    def _sighet_router(self):
        """Router whose factory hands every non-primary device its own sig-het
        engine (the per-device comp replica, in miniature)."""
        made = {}
    ...
    def test_sig_het_shards_build_independent_references(self):
        """The core regression: two shards whose intra-shard slot ids BOTH
        start at zero must each take the fresh-build branch, and neither's
        stash may move when the other builds."""
    ...
    def test_sig_het_clear_in_model_fans_out_to_replicas(self):
        """A replica that keeps ``_in_model`` set makes the NEXT block's first
        setup silently take the patch branch."""
        holder = FakeMultiShardACA(self.PER_BAND, 6, 2, layout="blocked")
        router, primary, made = self._sighet_router()
        params = np.arange(6 * 9, dtype=float).reshape(6, 9)
    ...
    def test_sig_het_without_factory_still_refuses_multi_shard(self):
        """Un-wired routers must keep failing loudly: one shared engine cannot
        hold two shards' references."""
        holder = FakeMultiShardACA(self.PER_BAND, 6, 2, layout="blocked")
...
class WDMHolderContractTest(unittest.TestCase):
    """``_as_wdm_holder`` must REFUSE multi-shard holders.

    Without this the ``*_wdm`` kernels read ``linear_data_arr[0]`` -- shard
    0's buffer -- with GLOBAL row indices, so a multi-GPU WDM run returns
    silently wrong likelihoods (or reads past the end) instead of failing.
    This is the guard that makes the router's existence enforceable.
    """

    def test_multi_shard_holder_raises(self):
        try:
            from lisatools.chunked_het import WDMComputationsBase
        except (ImportError, ModuleNotFoundError) as exc:
            self.skipTest(f"chunked_het not available: {exc}")
        holder = FakeMultiShardACA((3, 8), 6, 2, layout="blocked")
        comp = WDMComputationsBase.__new__(WDMComputationsBase)
        with self.assertRaises(NotImplementedError) as ctx:
            comp._as_wdm_holder(holder)
        self.assertIn("single-shard", str(ctx.exception))
        self.assertIn("_RoutedBandEngine", str(ctx.exception))

    def test_single_shard_holder_passes_through(self):
        try:
            from lisatools.chunked_het import WDMComputationsBase
        except (ImportError, ModuleNotFoundError) as exc:
            self.skipTest(f"chunked_het not available: {exc}")
        holder = FakeMultiShardACA((3, 8), 4, 1, layout="blocked")
        comp = WDMComputationsBase.__new__(WDMComputationsBase)
        self.assertIs(comp._as_wdm_holder(holder), holder)
...
class GBCompReplicaContractTest(unittest.TestCase):
    ...
    ``device_local_gb_comp`` is a no-op with only one device to replicate
    onto, so the shard-router tests above can only prove the dispatch. This
    class proves the other half -- that the recorded reconstruction arguments
    actually rebuild an equivalent comp -- which is otherwise only
    discoverable on a multi-GPU node. A missing/renamed constructor argument
    would be a TypeError on the cluster and nowhere else.

    Small grid on purpose; the cost is dominated by orbit configuration.
    """
    ...
    def test_wdm_comp_rebuilds_from_recorded_args(self):
        ...
        # CPU records no device; on CUDA this is the device the buffers above
        # were allocated on and is what the router keys replicas by.
    ...
    def test_recorded_backend_is_the_resolved_one(self):
        """``force_backend`` must be recorded as the RESOLVED backend object.

        Recording the raw argument (``None`` in the common case) would make a
        replica re-resolve the PROCESS-WIDE backend, so a ``cpu``-forced comp
        on a GPU node would spawn cuda replicas -- the defect class of GBGPU
        ``3cf0f78`` / LAT ``9c224b3``, one level down.
        """
        comp = self._wdm_comp()
        self.assertEqual(comp.kwargs["force_backend"].name, comp.backend.name)
        replica = type(comp)(*comp.args, **comp.kwargs)
        self.assertEqual(replica.backend.name, comp.backend.name)
    ...
    def test_sighet_wrapper_rebuilds_from_its_knob_dict(self):
        ...
        instance's own ``_g`` grid/knob dict -- the mapping the replica helper
        uses must reproduce ``_g`` EXACTLY."""
        import inspect

        from gbgpu.gbsignalhetcomputations import GBSignalHetComputations

        from lisatools.utils.devicereplicas import _sighet_knobs

        comp = self._wdm_comp()
        sig = GBSignalHetComputations.for_band_engine(
            comp, nt_layer=8, n_sparse_fd=128)
        knobs = _sighet_knobs(sig, sig._g)

        # every for_band_engine keyword-only knob must be recoverable

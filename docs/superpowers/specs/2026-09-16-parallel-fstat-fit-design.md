...
3. **Stage A (comb reuse or comb scan + `select_comb_peaks`) stays on the head**, scored through the replicated row holder. Its epoch-1 wall time is logged and is the trigger for a later split; not in scope. — **SUPERSEDED 2026-09-18: the trigger fired (2804 s, 48% of epoch 1). See the addendum at the end of this file; only `select_comb_peaks` stays head-only.**
...
Stage A parallelisation (**done 2026-09-18 — see the addendum below**); centre-table parallelisation; VGB (no F-stat fit); the sub-band dispersal design (parked); WP8.

## Addendum 2026-09-18 — stage A parallelised (supersedes decision 3)

Decision 3 kept stage A on the head and made its epoch-1 wall "the trigger for a later split". The trigger fired. Epoch 1 of `gf_prod_6mo_v8_4gpu` measured **5883 s total: stage A's comb 2804 s (46.7 min) on ONE GPU with three idle, stage B 3068 s (51.1 min) over four ranks at 3.34x** — stage A was 48% of the epoch and the ceiling on the whole fit. Comb work at 6mo is 222,885,680 evals over six sky levels, 83% of them in the single nsky=512 level (362,020 nodes × 512). Per-rank sweep rate is ~75-81k evals/s and does not degrade when split (the measured stage-B result).

Stage A is now split by the SAME shape as stage B, so the two halves cannot drift apart:

3'. **Stage A is split per SKY LEVEL by CONTIGUOUS node range, equal node counts per compute rank, the head taking an equal share.** One command per level (new GB op `gb_fstat_comb`, inserted in `GB_OPS` between `gb_fstat_ref_row` and `gb_fstat_stage_b`, behind the same `_fstat_call` requirement). The level assignment itself (`nsky_per_node`) is a deterministic closed form of f0 alone, so every rank derives the identical one with NO communication; the head ships the level's node slice, not the assignment. Ranges come from the SAME `split_box_range` stage B uses — reused rather than re-derived, because its purity is what makes a resume reproduce the rank→range map. Contiguity is load-bearing for the same reason as decision 4: comb nodes are f0-ordered and the sig-het scorer rebuilds its one resident reference block on f0 crossings.

   New unit: `CombLevelSpec` + `run_comb_level`, the single entry point both the serial scan and every rank call (stage A's counterpart of `StageBGroupSpec` + `run_stage_b_group`). The spec slices `f0_nodes` only — the sky grid, `lv` and `mc_fix` define what a row MEANS and are never sliced, exactly as stage B never slices its Mc/alpha/sin-delta axes. Per-rank checkpoints `comb_nsky{lv}_r{rank}` sit beside the existing per-level ones under the same fingerprint discipline.

   **The per-node reduction happens on the rank, before anything goes to disk.** A rank returns `(F_max, best_alpha, best_sin_delta)` for its node range as one `(3, n)` float64 partial, not the block it scored: at 6mo level 6 that is 3 × 362,020 values instead of 185 million. The head concatenates in rank order and writes the SAME `*_comb.npz`; `select_comb_peaks` stays head-only and unchanged (a cheap reduction over the assembled `F_max`).

   `n_compute == 1` and the no-fan-out path run the existing serial `run_comb_scan` byte-identically — the same hard gate decision 8 sets for stage B. `_fstat_comb_runner` REFUSES to be built at one rank rather than renaming the legacy per-level checkpoint (which would throw away an in-flight comb — up to 47 min of scan) and round-tripping the gated serial sweep through disk.

**Also fixed here (it is the same code path):** the COMPLETED comb cache was reloaded on file existence alone and was NOT fingerprinted by the reference walker — `fingerprint_extra` salts `ckpt_fingerprint`, i.e. the in-flight progress files, and a finished `*_comb.npz` is not one. A refit whose global argmax moved re-selected its peak BOXES from the previous walker's scan while stage B scored inside them at the new reference. Ruled an acceptable limitation on 2026-09-17 (proposal quality, not correctness — births are MH-corrected) because a rescan cost the full 47-minute serial comb; epoch 1 then hit exactly that case (argmax moved from walker 0 to walker 2). With stage A split the rescan is ~12 min, so `run_comb_scan` stamps `fingerprint_extra` into the npz and `comb_cache_usable` refuses a mismatch. An UNSTAMPED (pre-branch) comb is reused with a warning, deliberately: refusing it would throw away a finished scan on the first relaunch of every fit in flight.

Expected after the split, at four ranks on the 6mo fit: stage A ~12-14 min, the epoch ~65 min against 98 measured, with stage B the dominant half again (~78%). Cluster gate: `docs/multirank-cluster-gates.md` Step 5, item 3.

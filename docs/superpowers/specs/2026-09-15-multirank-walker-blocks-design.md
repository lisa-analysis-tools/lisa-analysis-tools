...
- Commands (`gf_serve` dispatch), each entered via `_enter_rank_block` (pin device, `self.ntemps/self.nwalkers = ntemps/B`, clock sync of `time`/`num_proposals`/`_branch_propose_counts`, read-only tables `_cap_leaf_cap`/`_band_leaf_cap`/`_rj_band_shutoff`, `_reseed_firing`, `temper_vertical` override, per-rank `_ProposeTimer`) and left via `_exit_rank_block`; the head restores `self.nwalkers = N` after every fan-out:
  ...
  3. `gb_finish`: `_write_back_state` on the session sorter → export the WHOLE branch block (`block_coords (ntemps, B, nleaves_max, ndim)`, `block_inds (ntemps, B, nleaves_max)`; `None`/`None` for a neutral block) + `d_h/h_h (B, nleaves_max)` — the block, not just its alive leaves, because a `keep_all_inds` `BandSorter` aliases the branch array on CPU and leaves its rejected-birth fill in the DEAD slots, which `_propose_legacy` returns (fix round 4, 2026-09-16); second alive-only sorter → `get_band_info` → `band_counts (ntemps, B, num_bands)`; `check_ll_inject` → `log_like_final (B,)`; `_cap_stats_for_rank` if caps are on; teardown (`_buffer_cache_teardown`, `_fstat_nm_lanes = None`, session cleared).
- Head orchestrator (rewrite of 17109-17610, prologue 16849-17108 unchanged incl. early returns, reseed with `np.random.permutation(N)` on the full sub-state, cap arming, `setup()`): `new_state = GFState(state, copy=True)`; periodic wrap on the full branch; draw scan schedule; fan-out 1 → merge `log_like[0]`, counters; tempering gate 17316-17363 unchanged (head decides, ships the decision); if it fires: draw `tmp_start`, fan-out 2 → merge `log_like[0]`, SUM swap counters → `_adapt_band_temps` once; `_update_band_shutoff(_band_occupancy_cold_max)` (MAX over N) ; `self.time += 1` (17490 order preserved); fan-out 3 → write each block's whole branch slice into `work` (`work.coords[:, w0:w1]`, `work.inds[:, w0:w1]`; a neutral block is skipped and keeps what the head sliced), `sub.d_h/h_h`, `band_num_binaries` concat, `band_temps` write-back, `accumulate_proposals/accumulate_swaps` from summed counters, `new_state.log_like[:] = concat(log_like_final)`, `_update_band_leaf_caps(precomputed=concat)`; `accepted = zeros((engine_ntemps, N))`; propose-end logs from merged replies.

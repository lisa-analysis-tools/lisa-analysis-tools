#!/usr/bin/env python
# LAPTOP-SAFE VARIANT of gf_monitor_gen.py (2026-09-14). Same CLI, same page.
# Five deltas, all meant to be folded back into gf_monitor_gen.py once that
# file's pending edits from the other session are committed:
#   1. chain/gb is read ONE COLUMN at a time (f0) wherever a per-iteration
#      series needs it -- the stock ``[..., 1]`` slice pulls all nine
#      columns (7.6 GB on a 1061-row store) and gets the process killed
#      on an 8 GB laptop; peak here is ~2.8 GB.
#   2. GF_MONITOR_ARM_TAG env overrides the arm-cache tag (the run-dir
#      name gf_prod_3mo_v8_10walkers maps to 3mo_v8_10w, the PRE-noise-fix
#      arm; the noise-fix run must write 3mo_v8_10w_noisefix).
#   3. f0-derived per-iteration series (in-band leaf count, completeness vs
#      iteration, cell-occupancy ramp) are NaN-masked outside the reduced
...
if DET_F0 is not None:
    _f0_it = g["chain/gb"][:NIT, 0, 0, :, :, 1] * 1e-3     # (it, walker, leaf) -- f0 column only (LEAN: the [..., 1] form read all 9 columns = 7.6 GB on a 1061-row store)
    _in = (_f0_it >= DET_LO) & (_f0_it <= DET_HI) & gb_inds
    _cnt = _in.sum(axis=2).astype(float)                  # (it, walker)
    # LEAN/EXTRACT: f0 is a zero fill before the keep-window, so the in-band
    # count is 0 there -- mask it rather than draw a false jump.
    _cnt[: _pool_floor("gb")] = np.nan
    ax[0].plot(it, np.nanmax(_cnt, axis=1) if np.isfinite(_cnt).any() else _cnt.max(axis=1), color=CYAN, lw=1.6,
               label=f"{_mhz(DET_LO)}-{_mhz(DET_HI)} mHz (rows with real coords)")
...
if cap_cells is not None and cap_cells.size and cap_edges_arr is not None:
    ...
    def _cell_counts(iteration):
        ...
        f0 = g["chain/gb"][iteration, 0, 0, :, :, 1] * 1e-3       # mHz -> Hz (LEAN: f0 column only)
    ...
    _its, _occ, _tot = [], [], []
    _cap_its, _atcap = [], []
    _ramp0 = _pool_floor("gb")     # LEAN/EXTRACT: rows before this hold no coords
    for i in range(_ramp0, NIT):
        cc = _cell_counts(i)
    ...
    ax[1].set_title("occupancy vs the model" + (f" (rows {_ramp0}-{NIT-1}: extract keep-window)" if _ramp0 > 0 else ""))
...
if TRU is not None:
    ...
    ARM_TAG = os.environ.get("GF_MONITOR_ARM_TAG") or {
        "3mo_v3": "v3", "3mo_v4": "v4", "3mo": "v2"}.get(RUN_KIND, RUN_KIND)
    try:
    ...
    if SHOW_MATCH_STATS:
        ...
        for _tag in sorted(ARMS):
            ...
            _k = _x >= 0
            # LEAN/EXTRACT: rows where the arm had sources but none "in band"
            # are zero-filled coordinates (a reduced store), not zero recovery.
            _nm = np.asarray(_D["n_match"], float).copy()
            _nm[(np.asarray(_D["n_all"]) > 0) & (np.asarray(_D["n_band"]) == 0)] = np.nan
            ax.plot(_x[_k], 100 * _nm[_k] / NDET, color=_c, lw=2.0,
                    label=f"{_tag} completeness")
            axr.plot(_x[_k], 100 * _nm[_k]
        ...
        ax.set_ylim(0, 100); axr.set_ylim(0, 100)  # LEAN: 60 clipped every arm above 60% completeness

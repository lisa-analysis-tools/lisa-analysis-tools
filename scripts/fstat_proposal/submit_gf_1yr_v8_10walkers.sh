...
# ---- fill these in ---------------------------------------------------------
#SBATCH --job-name=gf1yr_v8          # job name
...
SLURM_LOG=/shared/data/global_fit_output/gf1yr_v8_${SLURM_JOB_ID:-manual}.log
LOG_MIRROR_PID=""
...
# would silently re-grid this run. Pin the 360-d production value.
# 1yr = 4x the 3-mo baseline in Nt: grid resolves Nf 1440 x Nt 8640 x dt 2.5
# (exact factor-2 of the 6-mo Nt 4320, factor-4 of 3-mo Nt 2160).
export TOBS_TARGET=31104000        # 360 d
# 1-yr sig-het layer stride (SAME 36-h stride parity the 3-mo default and the
# 6-mo=120 derivation use): 8640 h / 36 h = 240, and 240 divides Nt=8640
# evenly. NOTE: the old submit_gf_1yr_v5.sh used 270 (a 32-h stride); the
# 3mo_v8 header records that the finer-than-36-h grid "did not reproduce" a
# gain (node placement, not resolution, is the lever), so v8 stays at 36 h.
# 240 x GB_N_SUBBANDS=2048 = 4.9e5 resident cells -- fits, same class as the
# 3-mo 265x8192. Confirm from the log: grep "sig-het engine resolved" ...
# want nt_layer=240 (stride 36) ... sparse spacing 36.0 h.
...
# 1 yr: deeper confusion resolved (~2425 detectable at 1yr vs ~1064 at 3mo).
# 25000 is the linear extrapolation of the v8 lineage (3mo 10000, 6mo 15000,
# +5000 per 3 months) AND the value the proven old 1yr_v5 ran. Reducible to
# ~20000 if GPU memory is tight -- it is a hard recovery ceiling, so do not
# go below the expected alive-leaf count.
export GB_NLEAVES_MAX=25000
...
# walker, temp). Built 08-18, never exported here before: jobs 466-470 all
...
# domain-shape change is UNVERIFIED (fine here: this is a fresh 1yr store).
# REQUIRED at 1yr: the sig-het taper is Tobs-scaled, ceil(alpha/2 x Nt) = 44
# layers at 1yr, and 44 + 8 = 52 exceeds the default crop 20 -- the build
# guard (gb_no_fg.py) RAISES if taper+8 > crop. 60 clears it with 8 layers
# of margin (the crossover to failure is Tobs ~= 14.2 mo / the 23-mo script).
...
# 1yr: stash law is LINEAR in N_sparse_t, so halve twice from the 3-mo 4096
# (4096@3mo -> 2048@6mo -> 1024@1yr; same per-batch bytes as 3-mo). The
# full_pe revert is STRICTER at 1yr than the 3-mo "revert to 1024": the 1-yr
# chunked-het stash is ~29.7 MB/source, so ON RESUME INTO full_pe drop this
# to 256 (NOT 1024) AND re-enable both MEMPOOL_FREE=1 below -- this is the
...
# Verify on this run: in-model acceptance, the [GB_CELL_LL] and sig-het
# audit lines, and the inmodel_accept/gate/prior spans must all hold.
# GB_INMODEL_ACCEPT_KERNEL=0 restores the python chain.
...
# stands down -- job 476 logged "[GB_INMODEL_ACCEPT_KERNEL] GB_INMODEL_TRACE

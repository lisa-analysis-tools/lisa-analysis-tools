#!/bin/bash
# ============================================================================
# 6MO_V8_NOGB -- EXACT COPY of submit_gf_6mo_v8.sh MINUS the GBs and galfor
# (user ask 2026-09-11: "run for everything except the GBs and galfor. We
# will not inject them. But we will run everything else.").
#
# The WHOLE delta vs submit_gf_6mo_v8.sh (everything else byte-identical;
# diff the two files to verify):
#   * REMOVE_BRANCHES=gb,galfor -- run_combined_staged.py drops both
#     branches from the fit, every stage move list, and the gb stages
#     (stage ladder becomes source_search -> noise_search ->
#     noise_vgb_search -> full_pe over psd+vgb+sobbh+mbh+emri).
#   * SOURCE_TYPES pinned WITHOUT GB: the data carries NO galaxy stream
#     ("we will not inject them"), so there is no confusion foreground
#     and no galfor branch to fit it.
#   * GB_WARM_START_COMPONENTS= (explicitly empty): no gb branch => no
#     warm-start move, and the npz-existence FATAL below must not fire.
#   * Ladder preflight made gb-optional (no gb group in this store).
#   * Names: job gf6mo_v8_nogb, store gf_prod_6mo_v8_nogb.
# Every GB_* export below is retained verbatim for diff-parity with the
# parent script; without a gb branch they are read by nothing (VGB shares
# the band machinery and keeps the GB_TEMPER_* / mirror knobs it reads).
# ============================================================================
# ============================================================================
...
# ---- fill these in ---------------------------------------------------------
#SBATCH --job-name=gf6mo_v8_nogb     # job name
...
#SBATCH --output=/shared/data/global_fit_output/gf6mo_v8_nogb_%j.log     # combined stdout+stderr (captures [MAXLOGL]/[BENCH])
# ----------------------------------------------------------------------------
...
STORE_DIR=/shared/data/global_fit_output/gf_prod_6mo_v8_nogb/
...
SLURM_LOG=/shared/data/global_fit_output/gf6mo_v8_nogb_${SLURM_JOB_ID:-manual}.log
LOG_MIRROR_PID=""
...
# ---- NOGB DELTA (the whole point of this file) -----------------------------
...
with h5py.File(store, "r") as f:
    gb = f["global_fit"]["sub_backend"].get("gb")
    if gb is None:
        print("[LADDER] no gb branch in this store (nogb run); nothing to check.")
        raise SystemExit(0)
    bt = gb.get("band_temps")
    if bt is None:

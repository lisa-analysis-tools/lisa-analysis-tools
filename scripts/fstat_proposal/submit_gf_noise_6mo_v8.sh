#!/bin/bash
# ============================================================================
# NOISE-ONLY diagnostic -- 6-month Tobs (psd branch only; NO galfor, NO
# sources), against the mojito INSTRUMENT/L1 injection noise.
# 2026-09-21: 6mo twin of submit_gf_noise_v8.sh. Only STORE_DIR,
# BASE_FILE_NAME and TOBS_TARGET differ.
...
# ---- fill these in ---------------------------------------------------------
#SBATCH --job-name=gfnoise_v8          # job name
...
#SBATCH --output=/shared/data/global_fit_output/gfnoise_v8_%j.log     # combined stdout+stderr (captures [MAXLOGL]/[BENCH])
# ----------------------------------------------------------------------------
...
STORE_DIR=/shared/data/global_fit_output/gf_noise_6mo_v8/
...
SLURM_LOG=/shared/data/global_fit_output/gfnoise_v8_${SLURM_JOB_ID:-manual}.log
LOG_MIRROR_PID=""
...
export BASE_FILE_NAME=gf_noise_6mo
...
# would silently re-grid this run. Pin the 180-d value -- this is the ONLY
# physics difference from submit_gf_noise_v8.sh (2026-09-21): same noise-only
# variant, same PSD model, same exact-fine scoring, 6 months instead of 3, so
# the recovered Soms_d/Sa_a are directly comparable to the 6mo production arm.
export TOBS_TARGET=15552000
# NT IS THE KNOB THAT ACTUALLY RE-GRIDS THIS RUN. noise_mojito pins nf/nt
# explicitly, and EreborFit.wdm_grid returns nf*nt*dt whenever BOTH are set
# -- tobs_target is only consulted when one is None. So TOBS_TARGET alone
# leaves the 90-d grid in place and the run silently analyses 3 months.
# 1440 * 4320 * 2.5 s = 1.5552e7 s = 180 d, the SAME Nf/Nt the 6mo
# production arm runs (globalfit_run.log: "Nf=1440 Nt=4320").
export NF=1440
export NT=4320
...
# (data/INSTRUMENT/L1, the "NOISE" source type). NO galfor branch
# (INSTRUMENT stream has no galaxy), NO GB, NO VGB, NO GW sources --
# so a biased/mis-shaped PSD model shows up directly as a biased
# posterior against general.psd_injection (the reference truth, never
# injected). This is the honest end-to-end check the user asked for:
# does the PSD model recover the true injection instrument noise with
# nothing else in the data to bias it? The GB_* env knobs above are
# inert here (no gb branch). source_types defaults to ("NOISE",) --
# do NOT set SOURCE_TYPES/GB_ONLY.
mpiexec -n 3 python scripts/run_global.py --stock noise_mojito
# python scripts/run_global.py --stock noise_mojito   # single-process fallback

...
#   ** 1-YEAR VARIANT (2026-09-19). Derived from submit_gf_6mo_v8.sh the  **
#   ** same way that script was derived from the 3-month one: Tobs, ONLY    **
#   ** its derived settings, the in-window MBHB set, and the warm-start     **
#   ** source. Every changed number and its arithmetic:                     **
#   **                                                                      **
#   **   TOBS_TARGET      15552000 -> 31104000  (180 d -> 360 d)            **
#   **   grid             Nt 4320  -> 8640      (Nf 1440, dt 2.5 unchanged) **
#   **   SIGHET_NT_LAYER  120      -> 240       36-h stride parity:         **
#   **                                          8640 h / 238 = 36.3 h       **
#   **   GB_N_SUBBANDS    8192     -> 4096      byte product parity:        **
#   **                                          238 x 4096 = 0.975e6        **
#   **   GB_RJ_INMODEL_CHUNK 32768 -> 16384     1yr cells ~2x 6mo bytes     **
#   **   GB_NLEAVES_MAX   15000    -> 20000     deeper confusion resolved   **
#   **   MBHB_IDS         4 ids    -> 12 ids    every t_c < 360 d           **
#   **   warm start       3mo 10w  -> THIS 6mo 4-GPU run                    **
#   **   EDGE_CROP_WAVELETS 60 UNCHANGED -- the sig-het taper is a          **
#   **     FREQUENCY-side crop and Nf is unchanged at 1 yr. VERIFY on the   **
#   **     first build: the 6-mo note says the build guard RAISES if the    **
#   **     taper exceeds the crop, so a wrong value fails loudly, not       **
#   **     silently.                                                        **
#   **                                                                      **
#   ** ?? RESOURCES: 4 GPUs ARE MARGINAL AND THIS IS NOT A SAVING.          **
#   ** The byte-product and slab choices above give 6-MONTH PARITY, not     **
#   ** headroom -- and the 6-month run OOM'd TWICE on 2026-09-19, once in   **
#   ** bin_fold_real and once in the warm-start mixture logpdf. Both are    **
#   ** fixed on dev (the sig-het fold revert; the pair-chunked logpdf,      **
#   ** 31db37fc), and BOTH fixes are required here.                         **
#   ** On top of that this run carries 12 MBHBs instead of 4, and mbh_pe /  **
#   ** emri_pe are the only strictly LINEAR-in-rows branches, so the source **
#   ** block alone is ~3x the 6-month cost.                                 **
#   ** Before committing hours, run GF_LAYOUT_DRY_RUN=1 and watch           **
#   ** gpu_util_*.csv on the first iterations. If the peak approaches the   **
#   ** card, the documented levers in order are: GB_N_SUBBANDS 4096 -> 2048 **
#   ** (halves the slab, costs sequential refill passes), then more nodes.  **
#   ** Do NOT reach for SIGHET_NT_LAYER -- it is not a mid-store knob and   **
#   ** it MULTIPLIES with GB_N_SUBBANDS in the byte product above.          **
#   **                                                                      **
...
# 1-yr sig-het layer stride. SAME DERIVATION as the 6-mo 120, re-run at
# Nt=8640: the validated invariant is the ~36-h SPARSE STRIDE, not the
# layer count. N_sparse_t ~ nt_layer - 2, and the stride in hours is
# Tobs_h / N_sparse_t:
#     6 mo:  4320 h / 118 = 36.6 h   (nt_layer 120)
#     1 yr:  8640 h / 238 = 36.3 h   (nt_layer 240)  <- parity held
# 240 divides Nt=8640 exactly (8640/240 = 36), the same divisibility the
# 6-mo value has. Confirm from the log, do not assume:
#   grep "sig-het engine resolved" <store>/.../globalfit_run.log
# want: nt_layer=240 ... sparse spacing ~36 h
...
# 8192 -> 4096. The sig-het safety product is N_sparse_t x GB_N_SUBBANDS
# and SIGHET_NT_LAYER just doubled, so the subband count MUST halve to hold
# it:  6 mo 118 x 8192 = 0.97e6  ->  1 yr 238 x 4096 = 0.975e6, i.e. byte
# PARITY with the 6-month run, under the 23-mo 1.1e6 OK precedent. Slot
# bytes also scale with Tobs (~0.25 MB at 3 mo, ~0.5 at 6 mo, ~1.0 at 1 yr),
# so 4096 x ~1.0 MB ~ 4.1 GB of slab -- again 6-month parity, not a saving.
...
export GB_WARM_START_COMPONENTS=${GB_WARM_START_COMPONENTS-${STORE_DIR}/warmstart/gf_prod_6mo_v8_4gpu_refereed.npz}
...
# run Tobs at load.
export GB_WARM_START_SOURCE_STORE=${GB_WARM_START_SOURCE_STORE-/shared/data/global_fit_output/gf_prod_6mo_v8_4gpu/gf_prod_6mo_testing.h5}
...
# EVERY catalogue MBHB that merges inside the 360-d window, recomputed
# from mbhb_cat_mojito_lite_processed_MT_rounding_fixed.hdf5 via
# (TimeCoalescencePetersSSBFrame - (data_t0 - MOJITO_REFERENCE_TIME))/86400.
# The method reproduces the 6-month set {2,5,16,18} exactly.
#   id  18  t_c  91.5 d  SNR  634     id   9  t_c 278.4 d  SNR  811
#   id   5  t_c 101.6 d  SNR  227     id   0  t_c 296.4 d  SNR  245
#   id  16  t_c 110.0 d  SNR  554     id  15  t_c 305.0 d  SNR  528
#   id   2  t_c 168.0 d  SNR  607     id   3  t_c 335.8 d  SNR  530
#   id  12  t_c 236.7 d  SNR  466     id  10  t_c 357.1 d  SNR 1963
#   id   7  t_c 253.9 d  SNR 1367     id   4  t_c 278.8 d  SNR  978
# WATCH id 10: t_c 357.1 d leaves only 2.9 d of post-merger inside the
# window, so its ringdown is truncated. A 365-d window yields the SAME 12
# ids, so nothing is gained by stretching Tobs to catch it fully.
# 4 -> 12 MBHBs is a 3x branch cost, and mbh_pe is LINEAR in rows (a python
# loop around a single-source generator) -- see the resource note in the
# header.
export MBHB_IDS=0,2,3,4,5,7,9,10,12,15,16,18

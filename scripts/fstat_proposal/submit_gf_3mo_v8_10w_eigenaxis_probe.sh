...
# ---- fill these in ---------------------------------------------------------
#SBATCH --job-name=gf3mo_eigprobe    # job name
...
#SBATCH --output=/shared/data/global_fit_output/gf3mo_eigenprobe_%j.log     # combined stdout+stderr (captures [MAXLOGL]/[BENCH])
# ----------------------------------------------------------------------------
...
STORE_DIR=/shared/data/global_fit_output/gf_prod_3mo_v8_10w_eigenprobe/
...
SLURM_LOG=/shared/data/global_fit_output/gf3mo_eigenprobe_${SLURM_JOB_ID:-manual}.log
LOG_MIRROR_PID=""
...
# ---- sampler shape ---------------------------------------------------------
export NWALKERS=10                 # 10-walker variant of the v8 run (folder _10walkers)
...
# dead row visited every iteration -- in jobs 474/479 while this line said
# 0.1. The intended "half the birth attempts" against what actually ran is
# therefore 0.5; 0.1 would be a 10x cut of the search feed. PE moves DID
# carry 0.2 before, so 0.1 there is the intended halving.
...
# RUNNING-BACKUP CADENCE (2026-09-14): job 479 spent 60-80 s of every
# 5.4 min PE iteration in [SAVE] save_step -- the sampler's blocking
# handoff waiting for the saver rank to finish copying + fsyncing the
# 14 GB store into *_running_backup_copy.h5 after EVERY save (0.5 s at
# 1yr's 1.5 GB store). Every 10th save instead; the 600 s mid-iteration
# checkpoint still bounds a torn-store loss to that interval.
export BACKUP_ITER=10
# Joint noise+VGB max-lnL rider inside gb_search (2026-09-11; job 473
# measured ~6 rounds x 16 s = 93 s/it, 15% of the iteration). The [MAXLOGL]
# trace shows the foreground/PSD re-tracking lands in ROUND 1 of each GB
# iteration (IMPROVED jumps of hundreds of lnL); rounds 2+ add ~5 lnL per
# round, the tol-level wobble near the mode. CHECKS=1: keep taking rounds
# while a round still improves by > tol (5 lnL), stop at the first flat
# one -- adaptive, so a big residual change still gets every round it
# needs. ITERS_PER_STEP=0: no hard cap beyond the global 10-round ceiling.
# Consumed by run_combined_staged.py.
...
# 0.1 rad (2026-09-11, was 0 = refresh every drifted source): job 473
# refreshed 89% of sources at every 25-repeat check (34 s/it) while the
# end-of-block drift median was 0.55 rad over 100 repeats. Experiment A
# (windowed stash, CPU) sig-het error vs displacement from the reference:
# 0.02 rad 1.3e-8, 0.1 rad 2.4e-7, 0.3 rad 2e-6, 0.5 rad 5e-6 (x SNR^2/2)
# -- at 0.1 rad that is 0.004 lnL for SNR 184, far under the 0.07 median
# cold audit error. 1e-2 would refresh nearly everything (drift per
# 25-repeat check is ~0.25 rad median), i.e. no saving; 0.3 saves more but
# accuracy is the primary concern (user ruling), so 0.1.
# REVERTED to 0 (2026-09-11 evening): 0.1 rad refreshed 81% of sources
# instead of 89% at 3mo and 94% instead of 97% at 1yr -- no measurable
# wall-clock saving -- while the 1yr cold audit max rose 15 -> 22 in the
# same relaunch (cause not isolated). Accuracy is primary: back to 0.
...
# 1 (2026-09-11, was 0): the fused gate/accept kernels replace ~110-150
# array launches per repeat with 3 backend calls; decisions are documented
# bit-identical to the python chain (tests/test_gb_inmodel_accept_kernel.py).
# First production use -- verify on this run: in-model acceptance, the

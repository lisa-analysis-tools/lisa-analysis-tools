#!/bin/bash
# ============================================================================
# PRODUCTION global fit -- 6mo_v9_4gpu. EXACT COPY of submit_gf_6mo_v8.sh
# with exactly THREE families of changes (2026-09-24). Everything below the
# v9 block is v8 verbatim; diff the two files and the only hunks should be
# the ones listed here plus the naming.
#
#   V9-1. PER-SOURCE IN-MODEL CONVERGENCE (new, opt-in). The fixed newborn
#         repeat budget becomes a CEILING: each (temp, walker, band) row
#         runs until its own cell lnL stops climbing, then freezes; the
#         coldest half of the ladder decides when a band is done; a finished
#         band is swapped out for a queued one. See the GB_INMODEL_CONVERGE
#         block below and
#         docs/superpowers/specs/2026-09-24-gb-inmodel-convergence-design.md.
#         ⚠ AT THE SHIPPED WINDOW (100) THIS IS A SPEND, NOT A SAVING --
#         the floor is 101 repeats against v8's fixed 100. Budgeted
#         deliberately: sources finish climbing instead of being cut off.
#   V9-2. NOISE LIKELIHOOD BACK TO NORMAL (exact-fine). v8 ran the Q=8
#         time-coarsened delayed-acceptance surrogate; v9 runs the plain
...
# resumed under V9-2 -- both this script's [V8-NOISE] preflight and run.py
# refuse the mismatch. STORE_DIR below is already a new path.
#
...
#SBATCH --output=/shared/data/global_fit_output/gf6mo_v9_4gpu_%j.log     # combined stdout+stderr (captures [MAXLOGL]/[BENCH])
# ----------------------------------------------------------------------------

#!/bin/bash
# ============================================================================
# GPU-ROUTING VARIANT of submit_gf_6mo_v8.sh (2026-09-23).
#
# A MIRROR of submit_gf_6mo_v8.sh with exactly ONE family of changes: the
# rank-layout dispatch, so the generalized routing (n_compute = n_blocks x R,
# i.e. GPUs > walkers at any walker count) can actually be launched. Every
# physics knob, every Tobs-derived setting and every stage of the recipe is
# byte-identical to its parent and MUST stay that way -- this script exists
# because the parent is live production and is being edited by hand; forking
# it keeps the routing work off that file entirely.
#
# The five differences from submit_gf_6mo_v8.sh, and nothing else:
#
#   0. GF_GPU_ROUTING defaults to 1 HERE and only here. The unified routing
#      is OPT-IN in the library (src/.../communication/ranks.py), so merging
#      it changes no existing runbook: with the knob unset, build_layout
#      applies the pre-2026-09-23 rule exactly -- nwalkers must divide the
#      compute-rank count, with the one-walker replica carve-out. This
#      script is the variant that asks for the new rule, so it turns the
#      knob on and exports it. GF_GPU_ROUTING=0 ./this_script.sh falls all
#      the way back to the parent's layout behaviour, which is the rollback.
#   1. NGPUS accepts 8 / 16 / 32 as well as 2 / 4 (nodes = NGPUS/2, this
#      cluster's 2-GPU-per-node shape; on-demand above NGPUS=2 for the same
#      preemption-exposure reason the parent gives for NGPUS=4).
#   2. --distribution=block:block when RANKS_PER_BLOCK > 1, cyclic otherwise
#      (DIST= overrides). build_layout lays a block's R ranks out
#      CONTIGUOUSLY, so a blocked distribution co-locates them -- and their
#      per-unit delta-ledger allgather is latency-bound and fires once per
#      unit close, 9x per propose at GB_BAND_UNIT_STRIDE=9.
#   3. RANKS_PER_BLOCK is exported through the sbatch dispatch.
#   4. The NWALKERS ROUND-UP IS GONE, replaced by a layout REPORT. The
#      round-up enforced "nwalkers must divide n_compute", which the unified
#      factorization retired; silently rewriting the user's walker count
#      would now misreport the run (and a changed NWALKERS is refused by the
#      store on a resume anyway).
#
# To re-sync with the parent after it changes:
#   diff scripts/fstat_proposal/submit_gf_6mo_v8{,_gpurouting}.sh
# Everything outside those four blocks should be empty.
#
# ---------------------------------------------------------------------------
...
if [ -z "${SLURM_JOB_ID:-}" ]; then
  ...
  # THE OPT-IN. Off in the library, on in this variant -- see difference 0
  # in the header. Exported below so the in-job side sees it too.
  GF_GPU_ROUTING=${GF_GPU_ROUTING:-1}
  _k=${GPUS_PER_RANK:-1}
  case "${NGPUS}" in
    ...
    # 8/16/32 (2026-09-23): the unified layout makes GPUs > walkers legal,
    # so the counts the routing work targets must be launchable. Same
    # on-demand reasoning as NGPUS=4 -- the more nodes an MPI world spans,
    # the more preemption exposure it carries for the same work. 2 GPUs per
    # node is this cluster's shape, so nodes = NGPUS/2; override with NODES=.
    8|16|32) _NGPU_PART=${PARTITION:-gpu-80-ondemand}
             _NODES=$(( NGPUS / 2 )); _GRES=gpu:2 ;;
    *) echo "[SUBMIT] NGPUS=${NGPUS} unsupported (2, 4, 8, 16 or 32)."; exit 2 ;;
  esac
  ...
  _DIST_FLAG=""
  if [ "${_NODES}" -gt 1 ]; then
    # CYCLIC by default (the WP7 launcher ruling: it is what makes the
    # per-node GPU pools line up with the compute-rank ordering).
    #
    # BLOCK:BLOCK when a walker block is SHARED by several ranks
    # (RANKS_PER_BLOCK > 1). ``build_layout`` lays a block's R ranks out
    # CONTIGUOUSLY in compute-rank order, so a blocked distribution puts
    # them on one node -- and the per-unit delta-ledger allgather they run
    # is latency-bound and fires once per unit close (9x per propose at
    # GB_BAND_UNIT_STRIDE=9). Intra-node is ~1-2 us against ~5-10 us plus
    # fabric contention with every other group's simultaneous allgather.
    # DIST= overrides, which the cross-node replica gate needs.
    if [ "${RANKS_PER_BLOCK:-}" != "" ] && [ "${RANKS_PER_BLOCK}" -gt 1 ]; then
      _DIST_FLAG="--distribution=${DIST:-block:block}"
    else
      _DIST_FLAG="--distribution=${DIST:-cyclic}"
    fi
  fi
  exec sbatch --partition="${_NGPU_PART}" --gres="${_GRES}" --nodes="${_NODES}" \
       --ntasks="${NTASKS}" ${_DIST_FLAG} \
       --export=ALL,NGPUS="${NGPUS}",GPUS_PER_RANK="${GPUS_PER_RANK}",RANKS_PER_GPU="${RANKS_PER_GPU}",RANKS_PER_BLOCK="${RANKS_PER_BLOCK:-}",GF_GPU_ROUTING="${GF_GPU_ROUTING}",GF_LEGACY_RANK_LAYOUT="${GF_LEGACY_RANK_LAYOUT}" \
       "$0" "$@"
fi
...
#
# GF_GPU_ROUTING is the OPT-IN for the unified factorization and defaults to
# 1 in this variant only (header, difference 0). The same reasoning applies:
# a manual `sbatch <script>` skips the dispatch block, and without this the
# job would silently fall back to the legacy divisibility rule and refuse
# exactly the shapes this script exists to launch.
GF_GPU_ROUTING=${GF_GPU_ROUTING:-1}
export GPUS_PER_RANK RANKS_PER_GPU GF_GPU_ROUTING
_k=${GPUS_PER_RANK:-1}
if [ "${NGPUS:-2}" = "4" ]; then
  GF_LEGACY_RANK_LAYOUT=${GF_LEGACY_RANK_LAYOUT:-0}
else
...
# NO ROUND-UP ANY MORE (2026-09-23). ``NWALKERS`` used to be rounded up to a
# multiple of the compute-rank count, because ``build_layout`` demanded equal
# walker blocks and raised otherwise. Under the unified factorization
# ``n_compute = n_blocks x R`` with ``n_blocks = gcd(nwalkers, n_compute)``,
# EVERY pair resolves -- so silently changing the user's walker count would
# now be a lie about the run rather than a fix for it. (Changing NWALKERS on
# a resume is also refused by the store, which is the other reason not to.)
#
# What is left is a REPORT of the layout the engine will build, computed with
# the same gcd rule, so a surprising shape is visible in the submit log
# rather than only in the run's own [LAYOUT] line.
if [ "${GF_LEGACY_RANK_LAYOUT}" = "0" ] && [ "${N_COMPUTE_EFF}" -gt 0 ]; then
  _gcd() { a=$1; b=$2; while [ "$b" -ne 0 ]; do t=$b; b=$(( a % b )); a=$t; done; echo "$a"; }
  if [ "${RANKS_PER_BLOCK:-}" != "" ] && [ "${RANKS_PER_BLOCK}" -gt 0 ]; then
    _NBLOCKS=$(( N_COMPUTE_EFF / RANKS_PER_BLOCK )); _R=${RANKS_PER_BLOCK}
  else
    _NBLOCKS=$(_gcd "${NWALKERS}" "${N_COMPUTE_EFF}"); _R=$(( N_COMPUTE_EFF / _NBLOCKS ))
  fi
  if [ "${_NBLOCKS}" -gt 0 ] && [ $(( NWALKERS % _NBLOCKS )) -eq 0 ]; then
    echo "[SUBMIT] layout: NWALKERS=${NWALKERS} on N_COMPUTE=${N_COMPUTE_EFF} -> ${_NBLOCKS} block(s) of $(( NWALKERS / _NBLOCKS )) walker(s), ${_R} rank(s) per block"
    # The report computes the UNIFIED rule; build_layout only applies it when
    # the opt-in is on. Saying nothing here would print a layout the engine
    # is about to refuse -- the exact class of error this report exists to
    # surface. (R == 1 needs no opt-in: it IS the legacy walker-block rule.)
    _ROUTING_ON=0
    case "${GF_GPU_ROUTING:-}" in 1|true|True|TRUE|yes|on) _ROUTING_ON=1 ;; esac
    if [ "${_ROUTING_ON}" = "0" ] && [ "${_R}" -gt 1 ] && [ "${NWALKERS}" -gt 1 ]; then
      echo "[SUBMIT]   WARNING: GF_GPU_ROUTING is OFF, so build_layout will REFUSE this shape (the unified factorization is opt-in). Set GF_GPU_ROUTING=1."
    fi
    if [ "${NWALKERS}" -eq 1 ] && [ "${_R}" -gt 1 ]; then
      echo "[SUBMIT]   one-walker replica mode (every compute rank holds the walker; GB/VGB by band range, addremove/PSD by likelihood rows)"
    elif [ "${_R}" -gt 1 ]; then
      echo "[SUBMIT]   GPUs > walkers: GB/VGB disperse by band range; addremove scatters likelihood rows by walker"
    fi
    if [ "${_NBLOCKS}" -eq 1 ] && [ "${NWALKERS}" -gt 1 ] && [ "${N_COMPUTE_EFF}" -gt 1 ]; then
      echo "[SUBMIT]   WARNING: gcd(${NWALKERS}, ${N_COMPUTE_EFF})=1 -> ONE block replicated on every rank; no walker parallelism. Pick NWALKERS sharing a factor with ${N_COMPUTE_EFF}, or set RANKS_PER_BLOCK."
    fi
  else
    echo "[SUBMIT]   WARNING: RANKS_PER_BLOCK=${RANKS_PER_BLOCK:-} gives ${_NBLOCKS} block(s), which does not divide NWALKERS=${NWALKERS}; build_layout will refuse."
  fi
fi

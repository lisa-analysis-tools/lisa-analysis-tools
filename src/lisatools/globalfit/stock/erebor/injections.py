...
class SyntheticGBProcessingStep(BaseProcessingStep):
    ...
    Uses the chunked-heterodyne FD kernel (``gb_fd_fill_global``) -- the
    frequency-domain analog of the WDM comp the GB branch analyses with.
    ONE batched kernel accumulates every source into the global template on
    device, so this scales to a whole-galaxy injection; a per-source
    generator does not. It needs none of the legacy ``SharedMemoryGBGPU.cu``
    kernels, so it works against a GBGPU built with
    ``-DGBGPU_WITH_SHAREDMEM=OFF``.
    ...
    Args:
        ...
        oversample: Unused; kept for backwards compatibility with the
            legacy FD-kernel signature. The accuracy knob is now
            ``GB_SYNTH_N_SPARSE`` (heterodyne FFT length, default 2048 —
            see the convergence table in the source).
    ...
    def __init__(
        ...
        # Built by the chunked-heterodyne FD kernel (``gb_fd_fill_global``),
        # the frequency-domain analog of the WDM chunked-het comp the GB
        # branch analyses with. ONE batched kernel accumulates every source
        # into the global template, on device, so this scales to the whole
        # galaxy -- a per-source generator does not.
        #
        # This replaced the legacy ``GBGPU.generate_global_template`` in
        # 2026-08. Three things fall out:
        #
        ...
        #    data-template overlap cos(2*phi0)). The chunked-het kernels
        #    already ARE the analysis convention, so there is nothing to
        #    flip.
        #  * SHOULD close TODO(synthetic-amp-evolution): the ~0.7%-weighted
        #    residual between the synthetic stream and the analysis
        #    template (vgb 55-source initial ll ~ -82 vs ~0 on mojito) was
        #    attributed to generating the data with a DIFFERENT kernel than
        #    the one analysing it. Same family both sides removes that by
        ...
        #
        # t_ref = t_start matches what the analysis branch uses in synthetic
        # mode (gb_no_fg.py: ``gb.t0 = ... else synthetic_t_start``), and the
        # FD-het kernels require t_start == t_ref (unity phase factor).
        data_length = target_N // 2 + 1
        df = 1.0 / (target_N * dt)
        fd_settings = FDSettings(
            data_length, df, min_freq=0.0, max_freq=None,
            force_backend=force_backend,
        )
        # N_sparse is THE accuracy knob and the comp's own default (256) is
        # far too coarse for an injection. Measured against a converged
        # GBTDIonTheFly reference (itself exact: 16k vs 64k spline nodes
        # agree to 3e-19), |a-b|^2/|b|^2 on the stock 2-source table:
        #
        #     N_sparse    256     512    1024    2048    4096    8192
        #     Tobs =  4d  5.4e-3  2.7e-3 1.3e-3  6.6e-4  3.2e-4  1.5e-4
        #     Tobs = 30d                         1.6e-4  8.2e-5  4.1e-5
        #
        # First-order convergence (halves per doubling), and it gets BETTER
        ...
        # (num_templates=1, nchannels, n_rfft) complex accumulator: the
        # kernel sums every source into it in place.
        templates = xp.zeros(
            (1, nchannels_full, data_length), dtype=xp.complex128
        )
        # params columns are [A, f0, fdot, fddot, phi0, iota, psi, lam,
        # beta]. convert_to_ra_dec=False: the injection basis is ecliptic
        # lam/beta, matching the orbits above.
        comp.fill_global(
            xp.asarray(params), templates, convert_to_ra_dec=False,
        )

        # FD -> TD, on device when there is one. GB FD templates follow the
        # LDC convention (FD = dt * rfft(td)), so the inverse is
        # td = irfft(FD) / dt.
        td_signal = xp.fft.irfft(templates[0], n=target_N, axis=-1) / dt
        if hasattr(td_signal, "get"):
            td_signal = td_signal.get()
        td_signal = np.asarray(td_signal)[:nchannels]
